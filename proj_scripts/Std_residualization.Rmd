...
In the table below we can see that the effect size and SE are the same when we residualize both the dependent and independent variable.

```{r table FWL, echo=FALSE}
tab_model(m1, m3, show.se = T, show.ci = F, show.p = F)
```
...
tab_model(m1, m3, show.se = T, show.std = T, show.ci = F, show.p = F)
```
